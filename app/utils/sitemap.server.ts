...
<url>
  ...
  const rawSitemapEntries = (
    await Promise.all(
      Object.entries(remixContext.routeModules).map(async ([id, mod]) => {
        ...
        if (handle?.getSitemapEntries) return handle.getSitemapEntries(request)
        ...
        let path
        if (manifestEntry.path) path = removeTrailingSlash(manifestEntry.path)
        else if (manifestEntry.index) path = ''
        else return
  ...
  for (const entry of rawSitemapEntries) {
    ...
    } else sitemapEntries.push(entry)
  }
